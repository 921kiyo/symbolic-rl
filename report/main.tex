...
Once the input of the real experience is converted in ASP syntax, the agent should learn the rule the following definition of the model of the environment using ILASP. \\
...
The background knowldge is empty, and there are only positive examples in this task. Each example contains different transition history of the agent. Inclusions are valid moves and exclusions are invalid moves. Learning valid\_move is the same as learning the rule of the games (the model of the environment), and it is updated as the agent explores more in the environment.

In addition to the rule of the game, learning the following concepts will be crucial for transfer capability, as this concept will be applicable to any types of game environments.  \\
...
where it is assumed that the reward of -100 means losing the game or losing the agent's life. Once the agent has learned the concept of the game, it knows how to avoid the an obstable in the adjacent cell in a new environment. Example XX illustrates this transfer cabability. 


In addition, when the environment has been changed during the exploration, the agent creates a new model for this new environment. In two different but similar scenarios, the model is generalised based on the two models, and the agent's model is more general which covers both games. Thus in theory, the agent should still be able to exploit the efficient learning from the model-based learning, facilitating the transfer learning.
...
\section{Further Research and Brief Implementation Plan}
...
More research needs to be devoded in finalising the overall architecture, and the following issues needs to be considered. 
\begin{itemize}
...
\item Possibility of using other representational concepts such as \textit{Predictive Representations of State} or \textit{Affordance} [\cite{Sridharan2017}] for the agent. 

\item Preparation for a backup plan in case ILASP approach does not work.
...
Other learning methods will be Q-Learning and Deep Q-Learning since these methods are widely used as benchmark. 
