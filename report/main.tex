...
These problems motivate the use of function approximation, which estimates value function with function approximation. 
...
Stochastic gradient descent methods are commonly used for most of function approximation methods.

TODO EXPLAIN ONLINE VS OFFLINE LEARNING

SGD adjusts the weights vector by a fraction of alpha in the direction what will reduce the error on that example the most. Formally, it is defined as 
...
For linear method, there are different ways to represents states as features, such as polynomials, fourier basis, or radial basis functions to name a few. Feature construction depends on a problem you are solving. In this report, we describe \textit{Tile Coding} which will be used for our benchmark. 
