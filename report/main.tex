...
\label{rl}
Reinforcement learning (RL) is another subfield of mathine learning regarding an agent's behaviour in an environment in order to maximise defined total reward. The agent interacts with an envirnment (Figure \ref{agent_env}), and at each time step, an agent takes an action and receive observation, which affects the environment state and the reward (or penality) it receives from the action outcome. In this section, we briefly introduce necessary background in RL for our research. 
