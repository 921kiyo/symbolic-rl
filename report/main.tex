...
An agent interacts with an environment at a sequence of discrete time step, which is a part of the sequential history of observations, actions and rewards, which is of the form H\textsubscript{t} = O\textsubscript{1}, R\textsubscript{1}, A\textsubscript{1}, ..., A\textsubscript{t-1}, O\textsubscript{t}, R\textsubscript{t}.  A \textit{state} is a function of the history S\textsubscript{t} = f(H\textsubscript{t}), which determines the next environment.  A state S\textsubscript{t} is said to have Markov property if and only if
...
When an agent must make a sequence of decision, the sequential decision problem can be formalised using Markov decision process (MDP). MDPs formaly represent a fully observable environment of an agent for reinforcement learning.
...
\textit{value functions} estimate the expected return, or expected future rewarded,  for a given action in a given state. The expected reward for an agent is dependent on the agent's action. 

A solution to the sequential decision problem is called a \textit{policy $\pi$}, a sequence of actions that leads to a solution.
% which is a distributin over actions given states.

The state value function v\textsubscript{$\pi$}(s) under a policy $\pi$ , which is of the form: 
...
Also using deep neural network front-end might also cause a problem [TODO Refer to "One pixel attack for fooling deep neural networks"]. As demonstrated in XXX, a single pixel can dramatically influence the policy (Maybe not so important here??).
...
TODO "Relational Recurrent Neural Networks"

TODO "Relational Deep Reinforcement Learning"
...
The next phase of the individual project is implementation and experiments. We will implement the proposed architecture using Python and ILASPv3.1.0 (Clingo) (TODO INSERT REF), and compare the performance.

The platform we are planning to use to measure the performance of my approach will be GVG-AI Framework, which was created for the General Video Game AI Competition \footnote{http://www.gvgai.net/}, which provides game environments for an agent that should be able to play a wide variety of games without knowing which games are to be played.
...
Especially, we will measure the following aspects of the new Algorithms
